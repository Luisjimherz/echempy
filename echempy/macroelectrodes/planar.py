...
-*- coding: utf-8 -*-
This module is in developement phase. It is aimed to contain methods and functions
for electrochemically simulation under different numerical and experimental conditions.

Current status:
    It can solve difussional cyclic voltammetry for one-electron transfer processes
    by finite diferences numerical solution of Fick's diffusion laws. 
    
Theoretical considerations:
    The module is suitable for simulating diffusional cyclic voltammetry from an heterogeneous Nersntian reaction (A <-> B)
    where both species have equal diffusion coefficients, considering a single electron E-mechanism. The electrode is
    considered to be a planar macroelectrode, and both migration and convection were neglected assuming a strongly supported
    media within an hydorstatic system.
...
class Electrosystem:
    ...
    def export_con(self, filename):
        ...
        file =  open(filename, 'w', newline='')  #newline='' que la nueva fila de datos se inserterá inmediatemente después de la anterior
        with file:
            for key in self.expp.keys():
                file.write("%s: %s,"%(key, self.expp[key]))
            file.write('\n')
            for key in self.simp.keys():
                file.write("%s : %s, " %(key, self.simp[key]))
            file.write('\n')
...
class Cronoamperogram(Electrosystem):
    def __init__(self):
        Electrosystem.__init__(self)
    ...
    def export_camp(self, filename):
        ...
        file = open(filename, 'w',
                    newline='')  # newline='' que la nueva fila de datos se inserterá inmediatemente después de la anterior
        with file:
            for key in self.expp.keys():
                file.write("%s: %s," % (key, self.expp[key]))
            file.write('\n')
...
class Ciclyc_voltammetry(Voltammogram):
    def __init__(self):
    ...
    def fast(self, ei, es, scanr, DX = 1e-1, DT = 1e-6):
        """
        It solves the dimensionless difussional cyclic voltammetry of an oxidated species (A) of a reversible system
        under the following considerations:
            1. E-mechanism A (oxidated) <-> B (reduced)
            2. One-electron transfer proccess
        ...
        Numerical method: Explicit approach of finite differences with equidistant spatial, and temporal meshes ( Individual
        solution of concentration at each moment trhough an iterative algorithm)

        Parameters:
        ei: (Experimental) Starting potential
        es: (Experimental) Switching potential
        scanr: (Experimental) Potential's scan rate
        DX: (Accurracy) Length between spatial points
        DT: (Accurracy) Length between time points
        """
    ...
    def nernst(self, ei, es, scanr, DX = 1e-3, DT = 1e-6, omega = 1.1): # todo Clean
        """
        It solves the dimensionless difussional cyclic voltammetry of an oxidated species (A) of a reversible system
        under the following considerations:
            1. E-mechanism A (oxidated) <-> B (reduced)
        ...
        Numerical method: Implicit approach of finite differences with either equidistant or expansive spatial, grid and
         an equidistant time grid ( Solution of PDE -> Linear system trhough an LU Decomposition algorithm (Thomas algorithm))
        ...
        if omega == 1 :  # Spatial mesh
            n = int(cell_len / DX)
            lmbd = DT / DX ** 2  # Lambda parameter for numeric method
            alpha, beta, gamma = [-lmbd for s in range(len(n))], [1.0 + (2.0 * lmbd) for s in range(len(n))], [-lmbd for s in range(len(n))]  # Thomas coefficients
            Gamma = np.zeros(n)  # Modified gamma coefficient
            for i in range(1, n - 1):
                Gamma[i] = gamma / (beta - Gamma[i - 1] * alpha)
        else:
            h = DX
            Spatial_points = [0]
            ...
            Gamma = np.zeros(n) # Modified gamma coeffcient
            for i in range(1, n - 1):
                    Gamma[i] = gamma[i] / (beta[i] - Gamma[i - 1] * alpha[i])
        Concentration_space = np.ones(n)  # Concentration of sepecies A thorugh space
        Current = np.zeros(m)
        Voltage = np.zeros(m)
        Concentration = np.ones(m) # Concentration of species A thorugh time
        Time = [t for t in range(m)]
    ...
    def butlervolmer(self, ei , es, scanr, a = 0.5, k0 = 1e8, DX = 1e-3, DT = 1e-6, omega = 1.1):
       ...
       Numerical method: Implicit approach of finite differences with an expansive spatial, and an equidistant temporal
        grids ( Solution of PDE -> Linear system trhough an LU Decomposition algorithm (Thomas algorithm))

       Parameters:
       ei: (Experimental) Starting potential
       es: (Experimental) Switching potential
       scanr: (Experimental) Potential's scan rate
       a: (Kinetic) Simmetry factor
...
class Cronoamperometry(Cronoamperogram):
    def __init__(self):
        Cronoamperogram.__init__(self)
        self.experiment = 'Cronoamperometry'

    def cronoamp(self, e, Time, a = 0.5, k0 = 1e8, DT = 1e-12, DX = 1e-6, omega_x = 1.1, omega_t = 1.1):
        """
        It solves the dimensionless difussional cronoamperometry of an oxidated species (A) of reversible, quasi-reversible
        and non-reversible systems, under the following considerations :
           1. E-mechanism A (oxidated) <-> B (reduced)
           2. One-electron transfer proccess
           3. Reaction kinetics described by Butler-Volmer model
       ...
       PDE -> Linear system trhough an LU Decomposition algorithm (Thomas algorithm))
       ...
       k0: (Kinetic) Charge transfer coefficient
       DX: (Accurracy) Length between spatial points
       omega_x: (Accurracy) Expansive factor for spatial grid
       DT: (Accurracy) Length between time points
       ...
       """
        ...
        while Spatial_points[-1] < 6 * np.sqrt(Time):  # Diffusion_layer's length
            ...
            h = h * omega_x
        n = len(Spatial_points)
        # Cálculo de los coeficientes de Thomas
        alpha, beta, gamma = np.zeros(n), np.zeros(n), np.zeros(n) # Thomas coefficients
        for k in range(0, m - 1):
            for i in range(1, n - 1):
                ...
                beta[i] = 1 - alpha[i] - gamma[i]
        Gamma = np.zeros(n) # Modified gamma coefficient
        Concentration_space = np.zeros(n) # Concentration of sepecies A thorugh space
        Current = np.zeros(m)
        Concentration = np.zeros(m) # Concentration of sepecies A thorugh time
        for k in range(m):
            # Forward sweep
            ...
            for i in range(1, n - 1):
                Concentration_space[i] = (Concentration_space[i] - Concentration_space[i - 1] * alpha[i]) / (beta[i] - Gamma[i - 1] * alpha[i])              # Segundo paso "Back sustitution"
